...
class NLPEngine:
    ...
    def extract_user_flows(
        ...
        # ── Step 1: determine primary actor (only show the one that initiates flow) ─────────────────────────────────
        primary_actor = actors[0] if actors else {"actor": "user", "capabilities": []}
        actor_id = _id(primary_actor["actor"])
        lines_out.append(f'    {actor_id}(["{primary_actor["actor"].capitalize()}"])')
